...
The Linux artifact includes both `PyMermaid-Linux-x64.tar.gz` and a native `PyMermaid-Debian-Ubuntu-amd64.deb` package. Windows additionally provides `PyMermaid-Windows-x64-Setup.exe`, and macOS provides `PyMermaid-macOS-x64.dmg`. The Debian package targets amd64 and declares Graphviz and the required graphical libraries as dependencies.
